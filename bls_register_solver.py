...
# ── 调试开关 ─────────────────────────────────────────────────────────────────
USE_REQABLE_PROXY = True   # True: 使用 Reqable 本地代理（每分钟自动更换 IP）
REQABLE_PROXY_HOST = "127.0.0.1"
REQABLE_PROXY_PORT = 9000
DEBUG_NO_PROXY = False      # True: 不使用代理（直连，用于本地测试）
...
class BLSSession:
    ...
    def __init__(self, proxy=None):
        ...
        self.security_code = ""
        self.verify_token = ""
        self.captcha_id = ""           # RegisterUser 页面的原始 CaptchaId
        self._captcha_id_new = ""      # SubmitCaptcha 返回的新 CaptchaId（用于 Step5）
...
def step4_submit_captcha(
    ...
    status, resp_text, resp_headers = session.req(
        ...
        extra={
            ...
            "Origin": BASE_URL,
            "Referer": BASE_URL + "/CHN/CaptchaPublic/GenerateCaptcha?data=" + session.security_code,
        },
        timeout=30,
    )
...
def step5_send_otp(
    ...
    for attempt in range(1, max_retries + 1):
        ...
        # 使用 requests 直接发送（参考登录流程）
        url = BASE_URL + "/CHN/account/SendRegisterUserVerificationCode"
        kwargs = {
            "params": params,
            "headers": headers,
            "timeout": 30,
...
def solve_captcha_once(session: BLSSession) -> tuple[dict | None, str | None]:
    ...
    if result:
        ...
        # 保存 SubmitCaptcha 返回的新 captchaId（用于 Step5）
        session._captcha_id_new = new_captcha_id
        return result, None
...
def register_one_task(task_id: int, person: dict) -> dict:
    ...
    for otp_retry in range(1, 3):
        ...
        # 使用 SubmitCaptcha 返回的新 captchaId
        captcha_id_for_step5 = bls._captcha_id_new if bls._captcha_id_new else bls.captcha_id
        log(f"    [Step5] 使用 captchaId: {captcha_id_for_step5[:30]}...")
        otp_result, err = step5_send_otp(bls, email_addr, person["mobile"], captcha_data, captcha_id_for_step5, max_retries=2)
